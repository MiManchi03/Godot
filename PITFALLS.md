...
> 每次 AI 犯错后追加一条。
> 标签口径：[自动] 真拦截 / [半自动] 需人工介入 / [缓解] 降低概率但不拦截。
...
| D2 | 逻辑错误（改概率违反 SPEC 3.3） | 单测 | [自动] Phase 4 |
| D3 | 边界崩溃（空 / 负 / 超大输入） | 单测 | [自动] Phase 4 |
| D4 | 范围蔓延（顺手改无关文件） | 范围守卫 | [自动] Phase 3 |
| D5 | 虚假测试结论（谎称 tests passed，实际未运行） | 取证闸口 | [自动] Phase 3 |
| D6 | save schema 变更无迁移 | save 守卫 | [自动] Phase 5 |
...
- 现状：[半自动] 移除会清空文件的本地空白钩子后复测通过；提交前仍建议自行 stash
...
- 现状：[缓解] `run_gdlint.py` 跳过 > 1 MB 的脚本并给出提示
...
- 现状：[自动] `check_terminology.py` 增 `find_substring_conflicts` 自检，pre-commit 生效

### 契约不得声明尚未提交的接口
- 现象：`villager_system` 契约声明了 `get_villager_by_entity_id`，该方法仅存在于未提交 WIP，pre-commit 暂存后比对失败
- 处置：改为 `"pending": true` 声明（缺失只告警）；WIP 落地后移除 pending
- 现状：[半自动] 契约支持 `"pending": true`；需人工决定何时标 pending 与何时移除
